			...
			for (i = 0; i < rows; i=i+1)
    			begin
        			...
        			begin
            				...
            				begin
                				...
                				begin
                    					...
                    					begin
											...
											mult_B = SigmaMatrix[(c + k)][(c + l)];
											#1
											sum_g_g  = sum_g_g + mult_P;
											
											mult_A = image_in_r[y][z];
											mult_B = SigmaMatrix[(c + k)][(c + l)];
											#1
											sum_g_r  = sum_g_r + mult_P;
                    					end
                				end
            				end
							...
							$fdisplay(f,"%02h",result_b[i][j]);           //writing blue pixel values to output hex file
							$fdisplay(f,"%02h",result_g[i][j]);           //writing green pixel values to output hex file
							$fdisplay(f,"%02h",result_r[i][j]);           //writing red pixel values to output hex file
        			end
			end
